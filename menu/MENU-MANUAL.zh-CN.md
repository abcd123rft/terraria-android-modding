...
| **其它** | **解除召唤数量限制**（开关）：把**仆从**（`maxMinions`）与**哨兵/炮台**（`maxTurrets`）上限一起抬到 99，装备召唤槽（`slotsMinions`）同步；「读取当前召唤上限」按钮回显实际数值。<br>⚠ **两条游戏本身的硬规则改不了**（属于设计）：① **「唯一型」仆从**（星尘之龙等）永远只有一条 —— 多给的召唤槽让它**变长**而不是变多；② **哨兵/炮台同一类型只能存在一个**，再召唤是把原来那个挪过去 —— `maxTurrets` 只放开「不同类型哨兵的总数」。<br>这些字段**游戏每帧按装备重算**（实测一次性写 400ms 内就被改回 1），所以脚本每帧重写；在 `Player.ItemCheck`（真正检查召唤数量的地方）时刻实测读到 **99** ✓。<br>另有：打印内部状态到日志 / 图标图集状态 |
