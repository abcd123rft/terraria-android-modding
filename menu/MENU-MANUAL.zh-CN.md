...
| **背包**（原「物品」页） | **背包操作页**：**每次进入这一页自动读取一次背包**（59 格：0–9 快捷栏 / 10–49 主背包 / 50–53 钱币格 / 54–57 弹药格 / 58 其它），网格**按类型分区并配色**（快捷栏蓝底 / 钱币金底 / 弹药绿底），每格显示 图标 + 名字 + 槽号 + ×数量。<br>**页面只留一个「刷新背包」按钮**；其他操作全部在**点格子时于该格子旁边弹出**：`放入/替换物品`（物品面板选完写进这一格，钱币格/弹药格会提示对应类型）、`改数量`（内置键盘，按 maxStack 封顶）、`复制到空格`、`删除该格`、`⚠ 清空整个背包`（点两次确认）、`关闭`（点弹窗外空白处也能关）。<br>停在页面时每秒自动刷新（脏检查，只更新变了的格子）。**原「添加物品」已并入这里**（点格子 → 放入/替换物品）。<br>操作弹窗为**两列紧凑布局**，**贴着你点的那个格子**（下面放不下会自动翻到格子上方），点弹窗外空白处关闭。<br>（2026-09-19 修：① 建格重复 → **建前先清空容器 + 进入即置位的「进行中」防重入标记**（真因：Frida 的 JS 在 `Java.registerClass` 这类长 Java 调用期间会被 Java 线程重入，轮询趁机再进一次建格，两套网格交错）；② 弹窗定位被固定预留夹到同一位置 + 竖排按钮太高 → 改贴格子 + 两列布局；③ 结构异常自愈：每次刷新校验容器子节点数，对不上就重建；④ 弹窗贴不住格子（差 ~160px）→ 真因是**高度按行数估算**（估 438px，实际 314px），改成**先 measure 再定位**，现在弹窗底边与格子顶边只差 6px；⑤ **点「放入/替换」没效果** → 真因是按钮回调在 `for` 循环里用 `var` 建闭包，**捕获了循环变量**，同一行按钮最后都执行该行最后一个动作（行 `[放入/替换｜关闭]` 里点前者实际执行了「关闭」）→ 改用 `forEach` 每元素独立作用域；⑥ 背包弹窗补上菜单标记，脚本重载时能被 `sweep()` 清掉，避免残留全屏透明层吞掉点击；⑦ **弹药格/钱币格点「放入」时物品面板自动只列对应物品**（2026-09-20 加）：从**弹药格**（54–57）打开只列**弹药**（80 件），从**钱币格**（50–53）打开只列**钱币**（71–74 四枚）；此时隐藏分类条，改显示「只看弹药/只看钱币（N 件）」+「显示全部物品」一键取消筛选。物品集合由 `物品库/out/terraria_items_all.csv` 的「弹药ID」列**离线生成 ID 区间表**（`BAG_FILTER`）内嵌进脚本，零运行时开销、不读文件；搜索词与筛选叠加生效（例：弹药筛选里搜「箭」＝25 件）；⑧ **背包页只剩分区标题、一个格子都没有**（2026-09-20 修）：上一轮加的「预热」在标题界面就跑了，那时 `bagSlots()` 返回 0 → 只建出 5 个标题 + 16 个空行却把 `built` 置了位，自愈又因为子节点数正好等于期望值发现不了。现在：建格前必须 `bagSlots() > 0`（否则直接返回且不置 built）；自愈条件加「一个格子都没有但槽位存在 → 重建」；预热没成功时轮询会在**首次出现角色**时补建（正好落在读盘时，卡顿被盖住）） |
...
| **其它** | **解除召唤数量限制**（开关）：把**仆从**（`maxMinions`）与**哨兵/炮台**（`maxTurrets`）上限一起抬到 99，装备召唤槽（`slotsMinions`）同步；**特殊仆从也可多只（实验）**（开关）：把游戏用来记「每类只能一只」的那些布尔字段（`stardustMinion` 星尘龙 / `palworldFoxsparksMinion` 火绒狐 / `twinsMinion` / `spiderMinion` … 共 14 个）每帧清成 false。另有「读取当前召唤上限」按钮回显实际数值。<br>⚠ **两个游戏本身的硬规则改不了**（属于设计）：① **星尘之龙这类「唯一型」仆从永远只有一条**，多给的召唤槽会让它变长而不是变多（上面那个实验开关是绕过它的尝试，可能表现异常）；② **哨兵/炮台同一类型只能存在一个**，再召唤是把原来那个挪过去 —— `maxTurrets` 只放开「不同类型哨兵的总数」。另外这些字段**游戏每帧按装备重算**（实测一次性写 400ms 内就被改回 1），所以脚本每帧重写；在 `Player.ItemCheck`（真正检查召唤数量的地方）时刻实测读到 **99** ✓ |
...
### 7.6 图集的两个坑（2026-09-20 更正：之前只修对了一半）

**当时的现象**：按 `(X, Y, X+W, Y+H)` 裁出来的图标是「上面一截对、下面错位」的花屏。

**当时的结论（不完整）**：把 y 换成 `2048 − Y − H`，图标看着就"对"了。
**为什么没发现问题**：当时拿来验证的是铁阔剑、铁短剑、铁锤、蓝色手机 —— 这些图标
**上下近似对称**（剑、锤斜着看都像剑和锤），倒过来也看不出来。

**真正的原因（2026-09-20 查清）**：从 `resources.assets` 导出图集时漏了两步 ——
1. Unity 纹理是**自下而上**存的，写 PNG 时没把行翻回来 → **整幅图上下颠倒**；
2. 纹理字节序是 **BGRA**，当成 RGBA 写 → **红蓝通道互换**。

只把矩形 y 改成 `2048−Y−H`，只是把图块的**位置**找对了，图块**内容**仍是倒的、颜色仍是串的。
一眼能看出来的证据：**金币是蓝的、弱效治疗药水是蓝的、药水瓶是倒立的**。

**修法（现在采用）**：直接修素材本身 —— `物品图标/修正图集.py` 把图集上下翻转 + R/B 换回，
于是图集在**任何看图软件里都是正的**，脚本里的裁剪也回到最朴素的 `(X, Y, X+W, Y+H)`：
```js
var sx = r.x, sy = r.y;      // 素材已修正为正向，不再需要 2048-y-h
```
修完实测：金币金色、治疗药水红色且瓶口朝上、火把棕色木棍+橙色火苗。

**图集换了以后别忘了刷新设备缓存**：脚本优先读游戏私有缓存
（`/data/user/0/<包名>/cache/dsha_atlas_N.png`），缓存不会自动失效 ——
起 `图标HTTP服务.py`，然后在游戏里 `TERRARIA_SYS_MENU.icon.warm(true)` 强制重下，再重载脚本。
...
### 8.0 第二轮性能优化（2026-09-20，实测数字）

**结论先说**：真凶不是「每帧的钩子」（全开时也只占每秒 ~2.7ms），而是**重复创建 Android 视图**和
**每次字段访问都进一遍 il2cpp**。改完后：

| 操作 | 优化前 | 优化后 | 怎么改的 |
|---|---|---|---|
| 物品面板**再次打开** | 133 ms | **1.6 ms** | 面板改成「建一次、之后只 show/hide + 重置状态」（常驻弹层）；列表窗口不再每次重建 |
| 物品面板**首次打开** | 156 ms | 挂载后 2.2s **后台预热**，用户点开时 0 等待 | `CFG.prewarm`（可关） |
| 首次进**背包页** | 224–307 ms | **57 ms**（其余预热掉了） | 50 个常量 drawable 改缓存 + 挂载后 5s 后台预热背包网格 |
| 背包页**再次进入** | 53 ms | **9 ms** | 进页不再 `bagRefresh(true)` 全量重画 59 格，改走脏检查 |
| 背包刷新（59 格扫描） | 1.235 ms | **0.50 ms** | 字段直读 + 数字脏签名 |
| **数字键盘** | 26.7 ms | **6.8 ms** | 同样改成常驻复用 |
| 搜索**每敲一个字符** | 8.8 ms | **4.2 ms** | 小写名表缓存 + 件数文本/日志去重 + 列表格脏跳过 |
| 单次**字段写入** | 4.40 µs | **1.70 µs** | 见下「指针+偏移直读直写」 |
| 单次**字段读取** | 2.15 µs | **1.35 µs** | 同上 |
| **一帧 16 次写入** | 0.044 ms | **0.022 ms** | 同上（两个钩子合计每秒 5.2ms → 2.7ms） |

**① 字段「指针 + 偏移」直读直写（最大的一笔）**
`il2cpp_field_get_value/set_value` 每次 2–4µs，而每帧两个钩子要写十几个 Player 字段、背包每秒扫 59 格 ×3 字段。
IL2CPP 的实例字段本来就是「对象指针 + `FieldInfo.offset`」，所以把偏移缓存下来后直接读写内存
（约 0.05µs 的裸读写 + Frida 的 JS 绑定开销 ≈ 1.4µs，快 2–3 倍且**零 il2cpp 调用**）。
安全网：**每个字段第一次访问时用官方 API 读一遍对拍**，不一致就永久退回慢路径并写日志
（`字段直读校验不一致：xxx`）——布局假设一旦不成立会立刻暴露而不是静默写错内存。
只对 `Player`/`Item` 这两个实例类生效；`Main`/`Sandstorm` 等静态类字段仍走官方 API（另外做了字段+类型缓存，
省掉每次的 `toString()` 拼 key）。运行时统计：`TERRARIA_SYS_MENU.il.perf()` → `{fast, slow}`，慢路径应恒为 0。

**② Android 视图只建一次**
一次 Java 调用（`setText`/`new Button`/`setBackground`…）在本机实测 **0.13–0.28ms**，所以「重建 120 个视图」= 100ms+。
- 物品面板 / 数字键盘 → 常驻弹层（`overlay(title, true)`：关闭只 `setVisibility(GONE)`，下次 `show()` 复用）；
- 列表窗口（6 行 ×4 格）不再每次重建，只重填内容；
- 分类 chips / 标签页上色只重画**状态变了的那两个**；
- 背包格背景改成 6 个常量 drawable（原来建 59 个 GradientDrawable）；
- 列表格加脏检查（同一格同物品整格跳过，`setImageBitmap` 是最贵的一步）。

**③ 界面写入去重**：`say`/`setLabel`/`setVisible`/`setSwitch`/`setSeek` 全部「值没变就不碰 View」，
去重标记记在 **View 自己身上**（`v.__txt`）而不是模块级 map —— 否则面板关闭再打开时，
新 View 会被误判成「已经设过了」，表现为状态栏/标签空白（这个坑实测踩到过）。
时间条按 1/400 量化：游戏时间每秒推进 20 次不再每次都刷 View。

**④ 空闲零侵入**：所有持续类开关都关掉时 `playerIdle()` 直接返回 —— 钩子每帧只做 16 次布尔判断，
不写任何字段（实测全关状态下 3 秒只有 477 次字段访问，全部走快路径）。

**⑤ 排查开关**：`TERRARIA_SYS_MENU.cfg.prof = true` 打开分段计时（界面构建/列表计算/切页各段耗时写日志）；
`cfg.prewarm = false` 关掉后台预热。
...
**一句话**：路径分两类 —— **游戏进程内**只有 **2 个常量**要改；**容器侧脚本**优先用环境变量/命令行参数，
剩下 **5 个脚本**各写死一行路径（3 个按相对目录找、2 个是绝对路径），要么保持目录结构、要么改那一行。
