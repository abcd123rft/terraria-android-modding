...
| **背包**（原「物品」页） | **背包操作页**：进入即读取当前背包（59 格：0–9 快捷栏 / 10–49 主背包 / 50–58 钱币弹药），4 列图标网格显示**图标 + 名字 + 槽号 + 数量**，停在页面时每秒自动刷新（只更新变了的格子）。<br>**增**：`＋ 增加物品` → 物品面板（横向分类 + 系统输入法搜索）→ 内置键盘输数量 → 放进**选中的空格**或第一个空格；`替换为…` 用同一面板覆盖当前格。<br>**删**：`删除该格` / `⚠ 清空背包`（点两次确认）。<br>**改**：`改数量`（内置键盘，按物品最大堆叠自动封顶）/ `复制该格`（复制到第一个空格）。<br>**查**：`刷新背包` + 每秒自动刷新 + 点格子选中后在顶部显示「槽N：名字 ×数量（id）」。<br>（原「添加物品」已并入这里作为「增加物品」） |
