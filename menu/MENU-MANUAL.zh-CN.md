...
| **其它** | **解除召唤数量限制**（开关）：把**仆从**（`maxMinions`）与**哨兵/炮台**（`maxTurrets`）上限一起抬到 99，装备召唤槽（`slotsMinions`）同步；另有「读取当前召唤上限」按钮（回显仆从/哨兵/装备槽/当前仆从数）。⚠ 这三个字段**游戏每帧按装备重算** —— 实测一次性写 400ms 内就被改回 1，所以脚本每帧重写一次；在 `Player.ItemCheck`（游戏真正检查召唤数量的地方）时刻实测读到的是 **99**，即限制确实解除。宠物游戏只允许一只、没有数量字段可改。<br>另有：打印内部状态到日志 / 图标图集状态 |
